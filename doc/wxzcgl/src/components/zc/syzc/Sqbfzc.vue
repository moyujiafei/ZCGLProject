<template>
  <div>
    <div>
      <x-header :left-options="{showBack: true}">申请报废资产</x-header>
    </div>
    <grid>
    ...
    <flexbox class="foot">
      <flexbox-item>
        <x-button type="primary" @click.native="submitSQWXZC">确认</x-button>
      </flexbox-item>
      <flexbox-item>
    ...
    <group title="申请原因：">
      <x-textarea :max="100" v-model="sqbfRemark" ></x-textarea>
    </group>
  ...
  export default {
    ...
    methods: {
      ...
      submitSQWXZC () {
        ...
        api.post('/wx/gzwx/submitSyrBFSQ.do', {
          imgId: data,
          voiceId: _this.voice.serverId,
          zcId: _this.$route.query.zcId,
          remark: _this.sqbfRemark
        }).then((response) => {
          if (response === 'success') {
            this.$vux.toast.show({
              text: '成功'
