<template>
  <div>
    <div>
      <x-header :left-options="{showBack: true}">完成日常巡检</x-header>
    </div>
    <grid>
    ...
    <flexbox class="foot">
      <flexbox-item>
        <x-button type="primary" @click.native="finishRCXJ">确认</x-button>
      </flexbox-item>
      <flexbox-item>
...
<script>
  import { Grid, GridItem, GroupTitle, XHeader, Flexbox, FlexboxItem, XButton } from 'vux'
  ...
  export default {
    ...
    methods: {
      ...
      finishRCXJ () {
        ...
        api.post('/wx/gzwx/finishRCXJ.do', {
          imgId: data,
      ...
      uploadImage () {
        ...
        if (_this.images.localId.length === 0) {
          _this.$vux.alert.show({
            title: '',
            content: '请先使用 chooseImage 接口选择图片'
          })
          return
        }
        var i = 0
      ...
      playVoice () {
        ...
        if (_this.voice.localId === '') {
          _this.$vux.alert.show({
            title: '',
            content: '请先使用 startRecord 接口录制一段声音'
          })
          return
        }
        this.wx.playVoice({
