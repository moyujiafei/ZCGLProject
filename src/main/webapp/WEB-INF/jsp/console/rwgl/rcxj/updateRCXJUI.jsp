  ...
  <body>
	<div class="easyui-layout" data-options="fit: true">
                ...
                <div data-options="region:'north',iconCls:'icon-more',collapsible:false" style="height:150px;">
                	 <form id="updateRCXJ_Form" method="post">
                    <table  style="border-collapse:separate; border-spacing:0px 25px;" width="100%">
						<tr><td align="center">开始时间：<input id="kssj" name="kssj"/></td>
							...
							<td align="right">操作人：</td><td><input id="czr_RCXJ" name="czrmc" userid="${rw.czr}"/></td>
				  			<td align="right">验收人：</td><td><input id="ysr_RCXJ" name="ysrmc" userid="${rw.czr}"/></td>
				  			<td align="right">验收时间：</td><td><input id="yssj" name="yssj"/><span style="color:#FF0000;margin-left: 3px;">*</td>
						</tr>
	...
	<script type="text/javascript">
		...
		$("#confirm_RCXJ").click(function(){
			$.messager.progress();
			$("#updateRCXJ_Form").form('submit',{
				url: getContextPath()+"/console/rwgl/rcxj/updateRCXJ.do",
				...
				success: function(result){
					$.messager.progress('close');
					if(result=="success"){
						$.messager.alert("提示","更新日常巡检任务成功",'info');
						$("#updateRcxjDialog").dialog('close');
		...
		$("#czr_RCXJ").searchbox({
			...
			searcher: function(value,name){
				rcxjListOpt.queryWXUser("queryYsrDialog", "/console/wxgl/wxuser/queryWXUserUI.do", "查找验收人", {isEdit:true},"czr_RCXJ");
			},
			value: "${rw.czrmc}"
		});
		...
		$("#ysr_RCXJ").searchbox({
			...
			prompt: "请点击右侧图标",
			searcher: function(value,name){
				rcxjListOpt.queryWXUser("queryYsrDialog", "/console/wxgl/wxuser/queryWXUserUI.do", "查找验收人", {isEdit:true},"ysr_RCXJ");
			},
			value: "${rw.ysrmc}"
		});
        ...
        var zcidStr=JSON.parse("${zcidList}");
        if("${FinishedzcidList}"!=""){
        	var finishedZCidArray=JSON.parse("${FinishedzcidList}");
        }
		
		$("#rcxjZCList").datagrid({
			url: getContextPath()+"/console/rwgl/rcxj/getZCListByRWID.do",
			height: "100%",
			...
			columns: [[
			
				{
					...
					formatter: function(value,row,index){
						for(var i=0;i<zcidStr.length;i++){
							if(row.zcid==zcidStr[i]){
								$("#rcxjZCList").datagrid("checkRow",index);
							}
						}
					}
				},
