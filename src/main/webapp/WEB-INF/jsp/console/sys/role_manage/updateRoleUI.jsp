        ...
        <script type="text/javascript">
            $(function() {
                $('#tbRoleinfoName').textbox({
                    required: true,
                    validType: "remote['${pageContext.servletContext.contextPath}/console/role/checkRoleName.do?originalName=${currRoleInfo.name}','newName']",
                    invalidMessage: "角色名已存在",
                    value: "${currRoleInfo.name}"
                });
                $('#tbRoleinfoStatus').combobox({
                    ...
                    value: "${currRoleInfo.tybz}"
                });
                //加载权限分配树
                $('#rolePrivilegeTree').tree({
                    url: getContextPath() + "/console/role/roleMeunTree.do?role_id=${currRoleInfo.id}",
                    checkbox: true,
                    lines: true,
                });
                ...
                $("#lbRolenfoSave").linkbutton({
                    ...
                    onClick: function() {
                        ...
                        $("#roleInfoForm").form('submit', {
                            url: getContextPath() + "/console/role/updateRoleInfo.do",
                            onSubmit: function(param) {
								...
								param.id = "${currRoleInfo.id}";
								param.originalName = "${currRoleInfo.name}";
                            },
