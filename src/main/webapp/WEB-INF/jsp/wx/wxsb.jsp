	...
	<div class="page">
		...
		<div class="weui-cells">
			<div class="weui-cell">
				...
				<div id="wxsb_sbqk" class="weui-cell__ft"></div>
			</div>
			<div class="weui-cell">
				<div class="weui-cell__bd weui-cell__primary">
					<p>备注:</p>
			...
			<div class="weui-cells_form">
				<div class="weui-cell">
					<div class="weui-cell__bd">
						<textarea id="wxsb_remarkDiv" oninput="OnInput (event)" class="weui-textarea" placeholder="请输入文本" rows="3"></textarea>
...
<script type="text/javascript">
	...
	//设置内容
	var wxsb_sbqkDiv = document.querySelector("#wxsb_sbqk"),//DOM对象
	wxsb_remarkDiv = document.querySelector("#wxsb_remarkDiv"),
	inputCountSpan = document.querySelector("#inputCountSpan");
	wxsb_sbqkDiv.innerHTML = "${requestScope.sbzt}";
	function OnInput (event) {
		inputCountSpan.innerHTML = wxsb_remarkDiv.value.length;
	}
	...
	wx.ready(function() {
		...
		document.querySelector('#uploadImage').onclick = function() {
			...
			images.serverId = [];
			$(".page").append("<div id='wxsb_uploadMask' class='weui-mask'><img alt='' style='margin:62% 45%;text-align:center;' src='${pageContext.servletContext.contextPath }/images/loading.gif'></div>");
			function upload() {
				wx.uploadImage({
...
// 					alert('上传语音成功，serverId 为' + res.serverId);
		...
		function saveImageWXJL(serverId){
			...
			wx.getLocation({
				...
				success : function(res) {
					$.ajax({
						url: getContextPath () + "/wx/InsertWXSB_Wxjl.do",
						cache: false,
						async: true,
						data:{"rwid":"${requestScope.rwid}","jlwz":JSON.stringify(res),"sbzt_id":"${requestScope.sbzt_id}","media_type":"image","wx_media_id":serverId,"uuid":"${requestScope.uuid}","remark":wxsb_remarkDiv.value},
						dataType: "json",						
						success: function (result) {
							if (result == true) {
								//当上传多个图片时,弹出的wx_alert()不能按顺序执行
							}
						}, 
						error: function () {
							alert("Ajax error!");
			...
			wx.getLocation({
				...
				success : function(res) {
					$.ajax({
						...
						cache: false,
						async: false,
						data:{"rwid":"${requestScope.rwid}","jlwz":JSON.stringify(res),"sbzt_id":"${requestScope.sbzt_id}","media_type":"voice","wx_media_id":serverId,"uuid":"${requestScope.uuid}","remark":wxsb_remarkDiv.value},
						dataType: "json",						
						success: function (result) {
							if (result == true) {
